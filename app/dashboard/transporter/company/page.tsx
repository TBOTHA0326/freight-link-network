...
const UPLOAD_CATEGORIES: DocumentCategory[] = ["cipc", "tax_document", "id_document", "truck_registration", "brake_test", "roadworthy", "trailer_registration", "drivers_license", "pdp", "other"];
    ...
    <div className="space-y-6">
      ...
      <SectionCard title="Company Information">
        <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2 pt-2"><button type="submit" disabled={saving} className="bg-[#06082C] hover:bg-[#0a0e40] text-white font-semibold px-6 py-2.5 rounded-xl transition-colors disabled:opacity-50 flex items-center gap-2 text-sm">{saving ? <><Loader2 size={15} className="animate-spin" />Saving...</> : "Save Changes"}</button></div>
        </form>
      </SectionCard>
      <SectionCard title="Documents" subtitle="Company and fleet compliance documents">
        <div className="bg-gray-50 rounded-xl p-5 mb-5 space-y-4">
          ...
          <label className="flex items-center gap-3 w-full px-4 py-3 border-2 border-dashed border-gray-200 rounded-lg cursor-pointer hover:border-[#06082C] transition-colors">
            <Upload size={16} className="text-gray-400 flex-shrink-0" /><span className="text-sm text-gray-500 truncate">{uploadFile ? uploadFile.name : "Click to choose file..."}</span>
