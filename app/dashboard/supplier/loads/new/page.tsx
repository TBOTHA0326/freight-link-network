...
import { SA_PROVINCES, TRAILER_TYPE_LABELS, type TrailerType, type AddressSuggestion } from "@/database/types";
...
export default function NewLoadPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user || !profile?.company_id) { setError("Not authenticated."); return; }
    setLoading(true); setError(null);
    try {
      await createLoad(form, profile.company_id, user.id);
  ...
  return (
    <div>
      <PageHeader title="Post a Load" subtitle="Create a new freight posting" icon={PlusCircle}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-4">
          ...
          <button type="submit" disabled={loading} className="flex-1 bg-[#06082C] hover:bg-[#0a0e40] text-white font-semibold py-3 rounded-xl transition-colors disabled:opacity-50 flex items-center justify-center gap-2 text-sm">
            {loading ? <><Loader2 size={16} className="animate-spin" />Posting...</> : "Post Load"}
          </button>
        </div>
      </form>
    </div>
