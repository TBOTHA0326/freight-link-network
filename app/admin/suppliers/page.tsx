...
export default function AdminSuppliersPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setProfiles(all.filter((p) => p.role === "supplier"));
  ...
  return (
    <div>
      <PageHeader title="Suppliers" subtitle="All registered supplier companies" icon={Building2} />
      ...
      <SectionCard>
        <div className="mb-5 relative">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search suppliers..." className="w-full pl-9 pr-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#06082C]" />
        </div>
        {loading ? (
          <div className="flex items-center justify-center py-12"><Loader2 size={20} className="animate-spin text-gray-400" /></div>
        ) : filtered.length === 0 ? (
          <EmptyState icon={Building2} title="No suppliers found" message="No supplier accounts registered yet." />
        ) : (
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-3">
                      <Link href={`/admin/suppliers/${p.id}`} className="text-xs font-medium text-[#06082C] hover:underline">View →</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
